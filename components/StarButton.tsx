...
export function StarButton({ stars }: { stars: number | null }) {
  return (
    <Link
      ...
      target="_blank"
      rel="noreferrer"
      className="flex h-9 shrink-0 items-center gap-1.5 rounded-md border border-border bg-surface-elevated px-3 text-sm font-medium text-muted transition hover:border-value-green hover:text-foreground"
    >
      <GithubIcon />
      <span>Star</span>
